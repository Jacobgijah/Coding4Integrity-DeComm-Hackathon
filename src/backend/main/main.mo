...
actor class Main() {
    ...
    public func getTotalWalletValue(userName : Text) : async [Types.Price] {
        let userOpt = await getUserByName(userName);

        switch (userOpt) {
            case (?user) {
                let wallet = await user.getWallet();
                let aggregatedPrices = Buffer.Buffer<Types.Price>(0);

                // Iterate over wallet items and aggregate prices by currency
                for (price in wallet.vals()) {
                    var found = false;
                    for (i in aggregatedPrices.vals()) {
                        if (i.currency == price.currency) {
                            let newAmount = i.amount + price.amount;
                            let updatedPrice : Types.Price = {
                                currency = i.currency;
                                amount = newAmount;
                            };
                            // Remove the old entry
                            aggregatedPrices.filterEntries(
                                func(index : Nat, p : Types.Price) : Bool {
                                    p != i;
                                }
                            );
                            // Add the updated price back into the buffer
                            aggregatedPrices.add(updatedPrice);
                            found := true;
                        };
                    };
                    if (not found) {
                        aggregatedPrices.add(price);
                    };
                };

                return Buffer.toArray(aggregatedPrices);
            };
            case (null) {
...
public shared func getUserTransactions(userName : Text) : async [Types.Transaction] {
    let userTransactions = Buffer.Buffer<Types.Transaction>(0);

    for (transaction in transactionsArray.vals()) {
        let buyerID = await transaction.getBuyerID();
        let product = await getProductById(await transaction.getProductID());

        switch (product) {
            case (#ok(product)) {
                let sellerID = await product.getSellerID();

                if (Text.equal(userName, buyerID) or Text.equal(userName, sellerID)) {
                    let transactionType = await convertTransactionToType(transaction);
                    userTransactions.add(transactionType);
                };
            };
            case (#err(_)) {};
        }
    ...
    return Buffer.toArray(userTransactions);
};
